...
class PlayerTest {
    ...
    void should_return_the_score_of_15_and_the_symbol_of_spare_if_an_attempt_is_a_spare() {
        ...
        int result = aPlayer.getTotalSumAttempt(aFrame);

        //Then
        assertThat(result)
                .isEqualTo(SPARE.getValue());
    }
...
//          result => 41
    ...
    @Test
    void result_of_many_successive_strike_frame_equals_to_the_sum_of_the_3_attempts_of_the_next_frame_plus_the_sum_all_the_previous_strikes() {
...
//          result => 56
        ...
        Frame nextFrame = aPlayer.getFrame(currentFrame.getName().getId() + 1);
        Frame secondNextFrame = aPlayer.getFrame(nextFrame.getName().getId() + 1);
        Frame thirdNextFrame = aPlayer.getFrame(secondNextFrame.getName().getId() + 1);

        // Given
        aPlayer.knockPinsAndUpdateScoreAt(15, currentFrame, new Attempt(FIRST_ATTEMPT.getId(), FIRST_ATTEMPT));
        aPlayer.knockPinsAndUpdateScoreAt(15, nextFrame, new Attempt(FIRST_ATTEMPT.getId(), FIRST_ATTEMPT));
        aPlayer.knockPinsAndUpdateScoreAt(15, secondNextFrame, new Attempt(FIRST_ATTEMPT.getId(), FIRST_ATTEMPT));
        aPlayer.knockPinsAndUpdateScoreAt(8, thirdNextFrame, new Attempt(FIRST_ATTEMPT.getId(), FIRST_ATTEMPT));
        aPlayer.knockPinsAndUpdateScoreAt(2, thirdNextFrame, new Attempt(SECOND_ATTEMPT.getId(), SECOND_ATTEMPT));
        aPlayer.knockPinsAndUpdateScoreAt(1, thirdNextFrame, new Attempt(THIRD_ATTEMPT.getId(), THIRD_ATTEMPT));
